...
cat(sprintf("Mean: %.4f\n", resultados_simulacao$media_ari)) # 0.324
cat(sprintf("Standard Deviation: %.4f\n\n", resultados_simulacao$dp_ari)) # 0.057

cat("--- Adjusted Mutual Information (AMI) ---\n")
cat(sprintf("Mean: %.4f\n", resultados_simulacao$media_ami)) # 0.337
cat(sprintf("Standard Deviation: %.4f\n", resultados_simulacao$dp_ami)) # 0.060